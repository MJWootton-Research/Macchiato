...
class PetriNet(object):
    ...
    time : integer
        By default, this is the UNIX timestamp when the Petri Net was created.
        It is also used a label for sequential simulations by the 'repeat' method
    units : string
        Unit of time to be used, essentially aesthetic (Default = 'hrs')
    savedot : boolean
        Write dot files (Default = False)
    visualise : string
        Toggle image output file type eg: png, svg... (Default = None for no images/.dots only)
    details : boolean
        Toggle simulation details on visualisation
    useGroup : boolean
        Group objects by group for visualisation, if provided
        Group is just a label for visualisation purposes - note trans and places have separte groups
    orientation : string
    ...
    *  schedule : Schedule based Monte Carlo integration (Default)
    ...
    schedule : collections.OrderedDict
        Stores the transition firing scheudle for the 'scheudle' Monte Carlo simulation run mode
    debug : boolean
    ...
    connectivity : list
        Descibes the network of connections between places and transitions (not implemented)
    step : integer
    ...
    arcsVerified : boolean
        Indicates if all arcs have been checked to see if their places are valid
    placeExit : boolean
        Indictes if simulated ended due a place's token limit being exceeded
    transExit : boolean
        Indictes if simulated ended due a transition firing the maximum number of times
    ...
    def addPlace(self, label, tokens=0, min=0, max=None, limits=None, group=None):
        ...
        max : integer
            Maximum number of tokens that this place can hold (Default = inf, parsed as None)
        limits : list of length two
            Simulation is terminated if number of tokens is outside limits[0] to limits[1].
    ...
    def addTrans(self, label, rate=None, uniform=None, delay=None, weibull=None, beta=None, lognorm=None, cyclic=None, maxFire=None, reset=[], vote=None, group=None):
        ...
        lognorm : list
            ...
            mu & sigma
        cyclic : list
            Length two list of parameters for a cyclic distribution (both floats)
    ...
    def resolveConflicts(self):
        """
        Removes ready to fire status from one of two conflicting transitions (chosen at random)
        Only used when runMode = 'all'.
    ...
    def selection(self, mode):
        """
        Constructs a Monte Carlo event table for ready to fire transitions, according
        to run mode, and returns a transition.
        ...
        *  schedule : Next scheudled transition fires
        ...
        time : float
            The clock advancement generated (only apllies to 'stochastic' mode)
    ...
    def getWait(self, trans):
        """
        Calculates the duration between the requisites of a transition being met and it firing, based on
...
class Place(object):
    ...
    limits : list of length two
        Simulation is terminated if number of tokens is outside limits[0] to limits[1]. Use 'None'
        entries for no bound
    ins : integer
    ...
    def checkLimits(self):
        ...
        ----------
        status : boolean
            True if number of tokens is outside mandated limits, False otherwise
    ...
    def changeTokens(self, n, safeMode=True):
        """
        Changes the number of tokens on a place while preserving ins and outs attributes.
        -- To be used for changes made to the net outside of normal simulation
        ...
        safeMode : boolean
        * If True, token count cannot be set below zero (negative number default to zero).
...
class Trans(object):
    ...
    weibull : list
        Length two or three list of parameters for a Weibull distribution (all floats)
        eta & beta, such that t = eta[-ln(X)]^-beta, and 0 < X < 1, sigma for optional uncertainty
    beta : list
        Length two list of parameters for a Beta distribution (both floats)
        alpha & beta, with optional third scale parameter
    lognorm : list
    ...
    cyclic : list
        ...
        frequency and offset
    maxFire : integer
        Maximum number of times the transition may fire before simulation is halted
    reset : list
        A list of places to reset on firing
    vote : integer
        Minimum number of incoming arcs required for voting behaviour(Default = None - i.e. no voting behaviour)
    waiting : list
    ...
    def checkMax(self):
        """
        Returns true if the maximum number of firings of this transition has been reached
...
class Arc(object):
    ...
    weight : integer
        Number of tokens consumed/created by arc (becomes float for place conditional arcs)
    type : string
        *  std : Standard Arc
        *  inh : Inhibit Arc
...
def repeat(pn, maxClock, maxSteps=1E12, simsFactor=1.5E3, fixedNumber=None, history=True, fileOutput=True, endOnly=False, analysisStep=1E2):#, log=True):
    ...
    simsFactor : float
        Parametises the number of simulations conducted. Repetition of simulations ends once the
        total simulated time supasses the product of maxClock and simsFactor.
    history : boolean
